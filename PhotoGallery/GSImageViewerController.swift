...
open class GSImageViewerController: UIViewController {
    ...
    @objc fileprivate func pan(_ gesture: UIPanGestureRecognizer) {
        ...
        func getProgress() -> CGFloat {
            ...
            print("width",width)
            print("Height",height)
            print("changeX",scrollView.center.x - origin.x)
            print("changeY",scrollView.center.y - origin.y)
            print("----------------------")
        ...
        func dismissView() -> CGFloat {
            let origin = panViewOrigin!
            print("Y",scrollView.center.y - origin.y)
            let Y = scrollView.center.y - origin.y
            return Y
        }
        
        func swipNext() -> CGFloat {
            let origin = panViewOrigin!
            print("X",scrollView.center.x - origin.x)
            let X = scrollView.center.x - origin.x
            return X
        }
        ...
        case .ended:
            if dismissView() > 200 {
                ...
                if indexVal < count-1 {
                    indexVal = indexVal + 1
                    imageView.image = UIImage(named: "\(imageList[indexVal])")
                    imageView.contentMode = .scaleAspectFit
                    scrollView.addSubview(imageView)
                    fallthrough
                }else{
